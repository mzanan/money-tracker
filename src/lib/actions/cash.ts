...
export async function markAsCashWithdrawal(
  txId: string,
  ...
  if (tx.kind !== "expense") {
    return { ok: false, error: "Only expenses can be cash withdrawals" };
  }
  if (tx.source === "manual") {
    return { ok: false, error: "This is already a cash entry" };
  }
  ...
  let ctx;
  try {
    ctx = await buildContext(user.id);
  } catch (error) {
...
export async function unmarkCashWithdrawal(
  txId: string,
): Promise<ActionResult> {
  const user = await getUser();
  if (!user) return { ok: false, error: "Not authenticated" };
      ...
      await dbTx
        ...
        .where(
          and(
            ...
            eq(transactions.external_id, `transfer:${txId}`),
          ),
        );
      await dbTx
        .update(transactions)
